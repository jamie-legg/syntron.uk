...
mkdir -p syn_uk_settings/pub/maps

# Create var directory
mkdir -p syn_uk_settings/pub/var

# Default output consolelog
touch syn_uk_settings/pub/var/consolelog.txt

# Default input input.txt
touch syn_uk_settings/pub/var/input.txt

# Create settings.cfg
cat > syn_uk_settings/pub/var/settings_custom.cfg <<EOL
RINCLUDE vov/games/sumo/bar_v1.cfg
EOL

cat > syn_uk_settings/pub/var/server_info.cfg <<EOL
...
# Create script.txt
cat > syn_uk_settings/pub/var/script.txt <<EOL
-- Custom scripting for syn uk sumo server
-- Add your custom scripting logic here
EOL

# Create command.txt
cat > syn_uk_settings/pub/var/command.txt <<EOL
-- Server commands to be executed on startup
-- Add your server commands here
EOL

# Create motd.txt
cat > syn_uk_settings/pub/var/motd.txt <<EOL
Welcome to syn uk Sumo Server!
Enjoy the game and have fun!
EOL


echo "Custom settings directory 'syn_uk_settings/pub' created with necessary files."