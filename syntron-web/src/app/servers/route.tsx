...
function parseColor(stringToParse: string): string {
  if(!stringToParse) return "";
    ...
    const cleanedName = stringToParse
        .replace(hexColorAndExtraRegex, '')  // Remove hex codes
...
export async function GET(request: Request) {
  try {
    const rawRanks = await axios.get("https://browser.armanelgtron.tk/servers_link/serverlist_v2.json");
    ...
    return new Response(JSON.stringify({ metadata, servers }), {
