...
export function ProfileScreen({ user }: { user: UserProfile }) {
  ...
  useEffect(() => {
    if (status === "authenticated") {
      getUserOverview(user.name).then((response) => {
    ...
    <div className="grid md:grid-cols-[300px_1fr] gap-8">
      <div className="flex flex-col items-start gap-6">
        ...
        <div className="grid gap-2 text-center">
          ...
          <p className="text-sm leading-relaxed text-sky-300 dark:text-sky-200"></p>
        </div>
      </div>
      <div className="grid gap-8 overflow-y-auto">
        <Tabs className="w-full" defaultValue="overview">
          ...
          <TabsContent className="pt-6 overflow-y-auto" value="overview">
            <div className="grid gap-4">
              <div className="grid gap-1">
                <h3 className="text-lg font-medium">About</h3>
                <p className="text-sky-300 dark:text-sky-200">
                  Jared is an avid gamer who has been part of the community for
                  over 5 years. He enjoys playing a variety of games, from
                  role-playing to strategy titles.
                </p>
              </div>
              <div className="grid gap-1">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge variant="secondary">
                    Won 25 strategy game tournaments
                  </Badge>
                </div>
              </div>
            </div>
          </TabsContent>
          <TabsContent className="pt-6 overflow-y-auto" value="match-history">
            <div className="grid gap-4">
              <div className="grid gap-2">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium">Recent Matches</h3>
                  <Link
                    className="text-sm text-gray-900 hover:underline dark:text-sky-30"
                    href="#"
                  >
                    View all
                  </Link>
                  ...
                  <TableBody>
                    {gameHistory.length > 0 &&
                      gameHistory.map((game, index) => {
                        const place = game.players[0].place;
                        const placeText = place === 1 ? "1st" : place === 2 ? "2nd" : place === 3 ? "3rd" : `${place}th`;
                        return (
                          <TableRow key={index}>
                            <TableCell className="uppercase text-sky-300 ">{game.region}/{game.server}</TableCell>
                            <TableCell>{placeText}</TableCell>
